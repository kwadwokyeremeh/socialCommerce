...
const axiosClient = () => {
    ...
    axiosCilentVersion.interceptors.request.use(async (request) => {
        ...
        if (!existingAuthHeader) {
            ...
            if (session && session.user) {
                ...
                request.headers.common['Authorization'] = `Bearer ${session.user.accessToken}`
            }
        }
     ...
     * @returns
     */
    const backendRequest = (method: string = 'get', url: string, body = null, cookies: string[]) => {
        // Cookies (CSRF) must be set, otherwise all requests will give unauth errors
        return axios.request({
            method: method,
            url: url,
            data: body,
            headers: createLaravelCSRFHeader(
                cookies,
            )
        })
            .then(res => res)
